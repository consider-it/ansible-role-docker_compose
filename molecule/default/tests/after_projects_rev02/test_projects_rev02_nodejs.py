def test_files(host):
    ...
    main_js_file = host.file("/ansible_docker_compose/projects/nodejs/app/main.js")
    assert main_js_file.is_file == True
    assert main_js_file.user == "root"
    assert main_js_file.group == "root"
    assert main_js_file.mode == 0o644

def test_endpoints(host):
    assert host.run("curl localhost:3000").stdout == "nodejs Docker Compose Project Rev 01"
